  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        className="flex items-center gap-1.5 px-2 py-1.5 text-sm font-medium text-gray-700 hover:text-gray-900 hover:bg-white rounded-lg transition-colors"
        aria-haspopup="listbox"
        aria-expanded={open}
        aria-label={t('language.label')}
      >
        <span>{t(LANGUAGE_LABEL_KEYS[active] ?? LANGUAGE_LABEL_KEYS.en)}</span>
        <ChevronDown
          size={14}
      ...
      {open && (
        <ul
          role="listbox"
          aria-label={t('language.label')}
          className="absolute right-0 mt-1 min-w-[9rem] py-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50"
        >
