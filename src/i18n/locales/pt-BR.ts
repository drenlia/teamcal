export const ptBR = {
  title: 'Agenda de Membros do Time',
  addTeam: {
    placeholder: 'Digite o nome do membro do time',
    button: 'Adicionar Membro'
  },
  timeSettings: {
    to: 'até',
    label: 'Horário padrão do turno'
  },
  timeInput: {
    hour: 'Hora',
    minute: 'Minuto',
    period: 'Período',
    am: 'AM',
    pm: 'PM',
    start: 'Hora de início',
    end: 'Hora de término',
    endBeforeStart: 'O horário de término deve ser depois do início no mesmo dia.',
    presets: {
      day: 'Dia (7h–15h)',
      standard: 'Padrão (9h–17h)',
      evening: 'Tarde (15h–23h)',
      night: 'Noite (23h–7h)'
    }
  },
  buttons: {
    print: 'Imprimir Calendário',
    cancel: 'Cancelar',
    save: 'Salvar',
    delete: 'Excluir',
  },
  warnings: {
    selectTeam: 'Por favor, selecione um membro do time antes de agendar'
  },
  calendar: {
    today: 'hoje',
    month: 'Mês',
    week: 'Semana',
    day: 'Dia',
    showOnlyMember: 'Mostrar apenas este membro no calendário',
    showAllMembers: 'Mostrar todos os membros',
    filterActive: 'Calendário filtrado para um membro.',
  },
  eventDialog: {
    title: 'Editar Horário do Evento',
    assignedTo: 'Atribuído a',
    start: 'Início',
    end: 'Fim',
    date: 'Data',
    quickShifts: 'Turnos rápidos',
    description: 'Descrição',
    viewDescription: 'Ver descrição',
    descriptionPlaceholder: 'Adicione a descrição do evento...',
    delete: 'Excluir turno',
    deleteConfirmTitle: 'Excluir este turno?',
    deleteConfirmMessage:
      'Este turno será removido da agenda. Esta ação não pode ser desfeita.',
  },
  language: {
    label: 'Idioma',
    en: 'English',
    pt: 'Português',
    fr: 'Français',
  },
  auth: {
    signIn: 'Entrar',
    signingIn: 'Entrando…',
    signInSubtitle: 'Entre para gerenciar a agenda do time',
    signOut: 'Sair',
    username: 'Usuário',
    password: 'Senha',
    newPassword: 'Nova senha',
    role: 'Função',
    roleAdmin: 'Administrador',
    roleMember: 'Membro',
    memberAccessTitle: 'Acesso do membro',
    memberAccessSubtitle: 'Definir credenciais de login para {{name}}',
    communicatePasswordReminder:
      'Lembre-se de compartilhar o usuário e a senha com segurança com esta pessoa após salvar.',
    doubleClickToManage: 'Clique duplo para gerenciar login e função',
    noLoginYet: 'Sem login',
    viewOnly: 'Somente leitura',
    credentialsSaved: 'Credenciais salvas',
    credentialsSaveError: 'Falha ao salvar credenciais',
    listAsMember: 'Listar como membro',
    listAsMemberHint: 'Desmarcado, a pessoa fica oculta na lista de membros (como admin).',
    notListed: 'Oculto da lista de membros',
    passwordKeepCurrent: 'Deixe em branco para manter a senha atual',
    passwordRequired: 'Senha obrigatória para novos membros',
    deleteMemberConfirmTitle: 'Remover este membro?',
    deleteMemberConfirmMessage:
      '{{name}} será removido da agenda junto com todos os turnos. Esta ação não pode ser desfeita.',
  },
  toolbar: {
    adminSubtitle: 'Gerencie membros, turnos e horários padrão',
    adminPanel: 'Horários padrão e gestão de membros',
  },
  demo: {
    resetCountdown: 'Esta demonstração será reiniciada em {{minutes}}m {{seconds}}s',
    loginTitle: 'Login de administrador da demonstração (preenchido abaixo)',
  },
  notifications: {
    loadError: 'Não foi possível carregar a agenda. Atualize a página.',
    teamAdded: 'Membro adicionado com sucesso',
    teamAddError: 'Falha ao adicionar membro',
    teamRemoved: 'Membro removido com sucesso',
    teamRemoveError: 'Falha ao remover membro',
    eventAdded: 'Evento adicionado com sucesso',
    eventAddError: 'Falha ao adicionar evento',
    eventRemoved: 'Evento removido com sucesso',
    eventRemoveError: 'Falha ao remover evento',
    eventUpdated: 'Evento atualizado com sucesso',
    eventUpdateError: 'Falha ao atualizar evento'
  }
};